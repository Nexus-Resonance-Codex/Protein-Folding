...
def run_nrc_pipeline(seq, viewer_type, folding_mode, ref_pdb_id=None):
    ...
    try:
        ...
        def safe_sub(arr, idx):
            return np.array(arr)[idx] if arr is not None else None

        # 3D Topology
        l_fig = go.Figure(
            data=[
                go.Scatter3d(
                    x=safe_sub(coords[:, 0], indices),
                    y=safe_sub(coords[:, 1], indices),
                    z=safe_sub(coords[:, 2], indices),
                    mode="lines+markers",
                    marker=dict(size=2, color=safe_sub(confidence, indices), colorscale="Viridis"),
        ...
        m_fig = go.Figure(
            data=[
                go.Scatter3d(
                    x=safe_sub(m_coords[:, 0], indices),
                    y=safe_sub(m_coords[:, 1], indices),
                    z=safe_sub(m_coords[:, 2], indices),
                    mode="lines",
        ...
        logs.append("[OK] FOLDING COMPLETE. MANIFOLD STABILIZED.")
        yield [
            "\n".join(logs),
            l_fig,
            m_fig,
