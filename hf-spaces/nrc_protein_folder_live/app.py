...
def process_folding(selection: str, custom_seq: str, steps: int, damping: float):
    ...
    if not seq:
        return (
            "ERROR: Missing Sequence Anchor", 
            None, None, None, None, 
            "Lattice Drift Detected: Re-anchor required."
        )
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            protein_select = gr.Dropdown(
                choices=["Custom Sequence"] + list(ProteinLibrary.DATA.keys()),
            ...
            sequence_input = gr.Textbox(
                placeholder="Enter raw FASTA or amino acid sequence...",
